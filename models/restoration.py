...
class DiffusiveRestoration:
    def __init__(self, diffusion, config):
        ...
        pretrained_model_path = self.config.args.resume + '.pth.tar'
        assert os.path.isfile(pretrained_model_path), ('Pre-trained diffusion model path is missing!')
        self.diffusion.load_ddm_ckpt(pretrained_model_path, ema=True)
        self.diffusion.model.eval()

    def restore(self, val_loader, r=None):
        image_folder = os.path.join(self.config.args.test_save_dir, self.config.data.dataset)
        with torch.no_grad():
