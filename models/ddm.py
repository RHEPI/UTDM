...
class DenoisingDiffusion(object):
    ...
    def train(self, DATASET):
        ...
        if os.path.isfile(self.config.args.resume):
            self.load_ddm_ckpt(self.config.args.resume)
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                x = x.flatten(start_dim=0, end_dim=1) if x.ndim == 5 else x     # 将Batch, Patch两个维度融合.
                n = x.size(0)
    ...
    def sample_image(self, x_cond, x, last=True, patch_locs=None, patch_size=None):
        skip = self.config.diffusion.num_diffusion_timesteps // self.config.args.sampling_timesteps
    ...
    def sample_validation_patches(self, val_loader, step):
        image_folder = os.path.join(self.config.args.val_save_dir, self.config.data.dataset + str(self.config.data.image_size))
        with torch.no_grad():
            ...
            x_cond = data_transform(x_cond)     # x_cond为有雾图像,x为高斯噪声.
            x = torch.randn(n, 3, self.config.data.image_size, self.config.data.image_size, device=self.device)     # 生成高斯分布噪音.
